...
struct KeaAssistantView: View {
    ...
    // Data model for the 4 main tabs + info
    let menuOptions: [(title: String, icon: String, description: String)] = [
        ...
        (
            "About Kea",
            "info.circle",
            "Kea is inspired by the Japanese loanword ケア (kea), derived from the English word \"care.\" It represents proactive self-health care; monitoring, maintaining, and improving physical and mental well-being. The app emphasizes preventive health through daily awareness, helping users build sustainable, healthy habits."
        )
    ]
...
struct ExpandableMenuOption: View {
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            if isExpanded {
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    // "Go to [Tab]" button (only show if it's not "About Kea")
                    if title != "About Kea" {
                        Button(action: {
                            navigateToTab(title)
                        }) {
                        ...
                        // Just add bottom padding for "About Kea" section
                        Spacer()
                            .frame(height: 16)
                    }
                }
    ...
    private func navigateToTab(_ tabName: String) {
        ...
        switch tabName {
        case "Dr Kea":
            selectedTab = 1  // FoodAdvisorView (Dr. Kea tab)
        case "Care Plans":
...
struct KeaAssistantView_Previews: PreviewProvider {
    static var previews: some View {
        KeaAssistantView(selectedTab: .constant(0))
    }
}
